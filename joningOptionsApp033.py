...
def set_font_size(size):
    ...
    for widget in [load_data1_button, load_data2_button, join_button, increase_font_button, decrease_font_button, sort_button, export_csv_button, export_json_button]:
...
# Create a PanedWindow for adjustable Data 1 and Data 2 sections
data_pane = tk.PanedWindow(root, orient="horizontal")
data_pane.grid(row=3, column=0, columnspan=7, padx=5, pady=5, sticky="nsew")

# Data 1 section with Treeview and filename label
data1_frame = tk.Frame(data_pane)
data1_file_label = tk.Label(data1_frame, textvariable=data1_filename, fg="gray", anchor="w")
data1_file_label.pack(anchor="w", padx=5)
data1_tree = ttk.Treeview(data1_frame, show='headings', style="Treeview")
data1_tree.pack(fill="both", expand=True)
data_pane.add(data1_frame)

# Data 2 section with Treeview and filename label
data2_frame = tk.Frame(data_pane)
data2_file_label = tk.Label(data2_frame, textvariable=data2_filename, fg="gray", anchor="w")
data2_file_label.pack(anchor="w", padx=5)
data2_tree = ttk.Treeview(data2_frame, show='headings', style="Treeview")
data2_tree.pack(fill="both", expand=True)
data_pane.add(data2_frame)
