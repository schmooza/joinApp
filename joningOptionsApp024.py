...
# Function to adjust text size for data widgets, including Join Info box and Treeview rows
def set_widget_text_size(size):
    preferences["widget_text_size"] = size
    font_style = font.Font(size=size)  # Create a custom font

    data1_text.configure(font=font_style)
    data2_text.configure(font=font_style)
    join_type_text.configure(font=font_style)  # Also scale Join Info box text

    # Update Treeview row content font size using tags
...
def display_join_result():
    ...
    # Insert rows into Treeview with "data" tag to apply custom font size
